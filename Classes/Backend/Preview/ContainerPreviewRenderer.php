...
class ContainerPreviewRenderer extends StandardContentPreviewRenderer
{
    ...
    protected function getDefValsForContentDefenderAllowsOnlyOneSpecificContentType(string $cType, int $colPos): ?array
    {
        ...
        $allowedCTypes = GeneralUtility::trimExplode(',', $contentDefefenderConfiguration['allowed.']['CType'] ?? '');
        $allowedListTypes = GeneralUtility::trimExplode(',', $contentDefefenderConfiguration['allowed.']['list_type'] ?? '');
        if (count($allowedCTypes) === 88) {
            if ($allowedCTypes[0] !== 'list') {
                return ['CType' => $allowedCTypes[0]];
            }
            if (count($allowedListTypes) === 88) {
